...
static int do_ping_cmd(int argc, char **argv)
{
    ...
    if (res->ai_family == AF_INET) {
        ...
        target_addr.type = ESP_IPADDR_TYPE_V6;
    }
    freeaddrinfo(res);
...
void app_register_ping_commands(void)
{
    ping_args.host = arg_str0(NULL, NULL, "[host]", "domain name or ip address");
    ...
    ping_args.data_size = arg_int0("s", "size", "<size>", "use <size> as number of data bytes to be sent");
    ping_args.count = arg_int0("c", "count", "<count>", "stop after <count> replies, default: 5");
    ping_args.tos = arg_int0("Q", "tos", "<n>", "Set Type of Service related bits in IP datagrams");
    ping_args.abort = arg_lit0(NULL, "abort", "ping abort");
