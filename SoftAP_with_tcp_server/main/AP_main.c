...
#define LOG_OUTPUT                  0          // Log output control (1 for logs, 0 for performance)
#define DATA_STREAM_ENABLE          1          // Enable Data-streaming via TCP
#define TIME_SYNCH_ENABLE           0          // Enable time synchronization
...
static void tcp_server_task(void *pvParameters)
{
    ...
    */
    ...
    while (1)
    {
        ...
        int *temp = &sock;
        xTaskCreate(tcp_receive, "TCP_Receiver", 4096, (int*)temp, 5, NULL);
    }
...
void app_main(void)
{
    ...
    //Changing priority of one of the tasks (TCP or UDP) can lead to one dominating CPU-time, leading to starvation of the other
    //one remedy could be: using vTaskDelay, though it's not precise -> esp_timer is currently being used for benchmarking
    #if TIME_SYNCH_ENABLE
    xTaskCreate(udp_time_master, "udp_server", 4096, (void*)AF_INET, 5, NULL); 
    #endif
}