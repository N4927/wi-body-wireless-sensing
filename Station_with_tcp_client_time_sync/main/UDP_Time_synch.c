...
void tcp_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        TickType_t wait = pdMS_TO_TICKS(50);   /* 50 ms queue poll */
        while (1) {
            uint8_t tok;
            ...
            send_full = false;   /* always reset – only one “1” frame */

            int sent = send(sock, packet, sizeof(packet), 0);
            if (sent < 0) {
                ESP_LOGE(TAG, "send errno %d – reconnecting", errno);
                break;          /* leave streaming loop, reopen socket */
            }
        }
        ...
        shutdown(sock, 0);
        close(sock);
        vTaskDelay(pdMS_TO_TICKS(1000));   /* 1 s back‑off */
    }
}
...
xTaskCreate(tcp_task, "tcp_task", 4096, NULL, 5, NULL);
    ...
    oppure:
    /***************************************************************
 * tcp_spike_dynamic.c – pacchetto 0x00 continuo,
 *                       0x01 nel primo giro dopo il trigger   *
 ***************************************************************/
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
...
#define TRIG_GPIO    18
#define NOTIF_TRIG   (1 << 0)          /* bit usato per il trigger */

#define SAMPLE_BYTES 3
#define NUM_CHANNELS 32
#define PAYLOAD_SIZE (SAMPLE_BYTES * NUM_CHANNELS)   /* 96 byte */
...
static const uint8_t hdr_tmpl[sizeof(PacketHeader)] = {
    BOARD_ID,
    0,0,0,0,
    (PAYLOAD_SIZE >> 8), (PAYLOAD_SIZE & 0xFF),
    0,0,0,0,0,0,0,0
};

static inline void build_packet(uint8_t *dst, const uint8_t *pl)
{
    memcpy(dst, hdr_tmpl, sizeof hdr_tmpl);
    uint32_t s   = htonl(seq++);
    uint64_t tsf = htonll(esp_wifi_get_tsf_time(WIFI_IF_STA));

    memcpy(dst + 1, &s , 4);
    memcpy(dst + 7, &tsf, 8);
    memcpy(dst + sizeof(PacketHeader), pl, PAYLOAD_SIZE);
}
...
void tcp_task(void *arg)
{
    ...
    /* GPIO 18: input + pull-up + interrupt fronte ascendente ------- */
    gpio_config_t cfg = {
    ...
    for (;;)
    {
        ...
        if (connect(sock, (void*)&dst, sizeof dst) < 0 &&
            errno != EINPROGRESS)
        {
            ESP_LOGW(TAG, "connect errno %d  retry", errno);
        ...
        for (;;)
        {
            ...
            const uint8_t *pl = spike_pending ? payload_one
                                              : payload_zero;
            spike_pending = false;
            build_packet(packet, pl);

            ssize_t wr = send(sock, packet, sizeof packet, MSG_DONTWAIT);
            if (wr < 0 && errno != EAGAIN) {
                ESP_LOGE(TAG, "send errno %dreconnect", errno);
                break;                         
            }
            ESP_LOGI(TAG, "sended packet full of %d", packet[20]);
